...
const ARCH_DEFAULTS: Required<ArchGroup> = { peak: 85, archHeight: 15, thickness: 45, falloff: 280 }

type PointerGroup = { enabled?: boolean; radius?: number; strength?: number }
const POINTER_DEFAULTS: Required<PointerGroup> = { enabled: true, radius: 200, strength: 40 }

function __OriginkitBase_PredictiveArc(props: Props) {
    const {
        className,
        style,
        background = "#f8f9fb",
    ...
    vRef.current = {
        ...
        accent: accentColor,
        high: highlight,
        density: Math.round(clampN(num(density, 52), 30, 200)),
        dotSize: clampN(num(dotSize, 55), 15, 200) / 100,
        speed: clampN(num(speed, 40), 0, 100) / 50,
        peak: clampN(num(arch_.peak, 85), 0, 100) / 100,
        archHeight: clampN(num(arch_.archHeight, 15), -100, 300) / 100,
        thickness: clampN(num(arch_.thickness, 45), 10, 300) / 100,
        falloff: clampN(num(arch_.falloff, 280), 50, 600) / 100,
        pointerEnabled: pointer_.enabled !== false,
        pointerRadius: clampN(num(pointer_.radius, 200), 40, 600),
        pointerStrength: clampN(num(pointer_.strength, 40), 0, 100) / 100,
    }
...
export function PredictiveArc(props: Record<string, unknown>) {
  return <__OriginkitBase_PredictiveArc {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}

export default PredictiveArc;