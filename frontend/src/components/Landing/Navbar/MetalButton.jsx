...
export function LiquidMetalButton({
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      if (plate) {
        if (on.press) {
          ...
          `;
        } else if (hoverTarget > 0.5) {
          plate.style.background = "#08090a";
          plate.style.boxShadow = `
    ...
    return () => {
      ...
      btn.removeEventListener("keyup", handleKeyUp);
    };
  }, []);

  const u = height / 516;
  const contentLen = String(children || text || "").length;
  const bw = width || Math.max(148, contentLen * 9.5 * (height / 42) + (showIcon ? 54 : 36) * (height / 42));
  const bh = height;
  const pad = 900 * u;
      ...
      >
        <div
          ...
          style={{
            ...
            `,
            transition:
              "box-shadow .38s cubic-bezier(.22,.61,.36,1), background .38s cubic-bezier(.22,.61,.36,1)",
          }}
          aria-hidden="true"
        />
        ...
        <button
          ...
          style={{
            ...
            color: "#fff",
            fontFamily:
              'var(--font-sans, "Instrument Sans", -apple-system, sans-serif)',
            fontWeight: 600,
            fontSize: "calc(185 * var(--u))",
            lineHeight: 1,
          ...
          {showIcon && (
            ...
            >
              <g stroke="currentColor" strokeWidth="15" strokeLinecap="round">
                <path d="M57.5 12 V103" />
                <path d="M12 57.5 H103" />
              </g>
            </svg>
          )}
          <span
...
export const MetalButton = LiquidMetalButton;
