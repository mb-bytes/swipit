...
export function MonoRoundedDonutChart({
  ...
  const totalAmount = chartData.reduce(
    (acc, item) => acc + (Number(item.amount) || Number(item.value) || 0),
    0,
  );
      ...
      <div className="flex items-center justify-between mb-1 shrink-0">
        <div>
          ...
          <div className="text-xl font-bold tracking-tight tabular-nums mt-0.5 font-sans">
            ...
            <span className="text-xs font-normal opacity-70">spend</span>
          </div>
        </div>
      </div>
        ...
        {chartData.length === 0 ? (
          ...
          <>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-center">
              <span className="text-sm font-bold tabular-nums font-sans leading-none text-neutral-900 dark:text-white">
                {hoverIndex !== null
                  ? `${chartData[hoverIndex]?.value}%`
                  : `${chartData.length}`}
              </span>
              <span className="text-[10px] font-medium tracking-tight tabular-nums font-sans leading-tight text-neutral-500 dark:text-neutral-400 max-w-[56px] truncate mt-0.5">
                {hoverIndex !== null
        ...
        {chartData.length === 0 ? (
          ...
          chartData.slice(0, 4).map((seg, idx) => (
            <div key={idx} className="flex items-center gap-1 min-w-0">
              ...
              />
              <span
                className={`truncate max-w-[70px] ${isDark ? "text-neutral-400" : "text-neutral-600"}`}
              >
                {seg.name}
              </span>
            </div>
