...
def _is_transaction_email(clean: str) -> bool:
    return bool(
        re.search(r"spent\s+(?:Rs\.?\s*)?(?:INR|USD|EUR|GBP|AED|\d)", clean, re.I)
        or re.search(r"payment\s+of\s+(?:Rs\.?\s*)?(?:INR|USD|EUR|GBP|AED|\d)", clean, re.I)
    )
...
def parse_federal(html_body: str) -> dict | None:
    ...
    spend_amount_match = re.search(
        r"spent\s+(?:Rs\.?\s*)?(INR|USD|EUR|GBP|AED)\s*([\d,]+\.?\d*)",
        clean,
        re.I,
    )
    if not spend_amount_match:
        spend_amount_match = re.search(
            r"spent\s+(?:Rs\.?\s*)?([\d,]+\.?\d*)\s*(INR|USD|EUR|GBP|AED)?",
            clean,
            re.I,
        )
        if spend_amount_match:
            raw_amount = spend_amount_match.group(1).replace(",", "")
            currency = (spend_amount_match.group(2) or "INR").upper()
    ...
    if amount is not None and merchant_match and date_match:
    ...
    payment_match = re.search(
        r"payment\s+of\s+(?:Rs\.?\s*)?(?:INR\s*)?([\d,]+\.?\d*)\s+towards\s+your\s+credit\s+card",
        clean,
        re.I,
    )
    ...
    if payment_match:
        ...
        transaction_date = datetime.strptime(payment_date_match.group(1), "%d-%m-%Y").date()
